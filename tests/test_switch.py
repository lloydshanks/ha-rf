...
class TestRPiRFSwitch:
    ...
    def test_switch_initialization_with_defaults(self):
        ...
        assert switch.unique_id is None
    ...
    def test_turn_on_multiple_codes(self):
        ...
        # Should send both codes
        assert self.mock_rfdevice.tx_code.call_count == 2
        self.mock_rfdevice.tx_code.assert_any_call(
            1111111, tx_proto=1, tx_pulselength=350, tx_length=24
        )
        self.mock_rfdevice.tx_code.assert_any_call(
            1111112, tx_proto=1, tx_pulselength=350, tx_length=24
        )
    ...
    def test_send_code_with_repetitions(self):
        """Test _send_code with signal repetitions."""
        ...
        # Should call tx_code 3 times due to repetitions
        assert self.mock_rfdevice.tx_code.call_count == 3

    def test_send_code_with_none_repetitions(self):
        """Test _send_code with None repetitions (uses device default)."""
        ...
        assert result is True
        # Should call tx_code once (device handles repetitions)
